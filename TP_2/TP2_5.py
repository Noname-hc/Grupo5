...
- Ejecutar: python3 combined_search_gui_unitastar.py
- Seleccioná A* en el menú para que A* minimice número de pasos (W no tiene coste 30 aquí).
"""
...
positions = {
    # Bloque A-E movido 2 posiciones a la derecha
    'A': (4,3), 'B': (5,3),
    'C': (4,2), 'D': (5,2), 'E': (6,2),
    # Resto de casillas
...
# Paredes: C-D, D-E, T-F, W-R (barrera debajo de W)
walls = {
    ('C','D'), ('D','C'),
    ('D','E'), ('E','D'),
    ('T','F'), ('F','T'),
...
# ordenar vecinos alfabéticamente para cumplir desempates
for n in adj: adj[n] = sorted(adj[n])

def step_cost(to_node):
    # usado para mostrar costo en DFS/Greedy si se desea, pero A* usará coste unitario
...
def dfs_events(start='I', goal='F'):
    ...
    while stack:
        ...
        to_push = [nb for nb in neighbors if nb not in visited and nb not in stack]
    ...
    while open_heap:
        ...
        for nb in adj[node]:
            if nb in visited or nb in open_set:
                continue
            came_from[nb] = node
            heapq.heappush(open_heap, (manhattan(nb), nb))
            open_set.add(nb)
            yield ('push', nb, node)
        yield ('frontier_snapshot', sorted(list(open_set), key=lambda n: (manhattan(n), n)))
    yield ('failed', None)
...
def astar_unitcost_events(start='I', goal='F'):
    """
    A* que trata cada paso con coste 1 (no se tiene en cuenta W=30).
    Heap entries: (f, g, node, counter) and frontier_f holds (f,g) per node to discard stale entries.
    """
    counter = 0
    open_heap = []
    ...
    heapq.heappush(open_heap, (fscore[start], gscore[start], start, counter))
    ...
    while open_heap:
        ...
        for nb in adj[node]:
            ...
            if tentative_g < gscore.get(nb, float('inf')):
                ...
                if nb not in closed:
                    counter += 1
                    heapq.heappush(open_heap, (fnb, tentative_g, nb, counter))
